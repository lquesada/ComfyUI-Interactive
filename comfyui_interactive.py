...
class InteractiveTriggerWithParameters:
    def __init__(self):
        self.output_dir = folder_paths.get_temp_directory()
        self.last_trigger_action = 0
        self.compress_level = 4
        self.prefix_append = "_trigger_" + ''.join(random.choice("abcdefghijklmnopqrstupvxyz") for _ in range(5))

    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "trigger_action": ("INT", {"default": 0}),
            },
            "optional": {
                "images": ("IMAGE", ),
    ...
    def run(self, trigger_action, images=None, parameter_latent=None, parameter_mask=None, parameter_string=None, parameter_int=None, parameter_float=None, prompt=None, extra_pnginfo=None):
        out = {}
        ...
        if trigger_action != self.last_trigger_action:
            self.last_trigger_action = trigger_action
        else:
            nodes.interrupt_processing()
        return (images, parameter_latent, parameter_mask, parameter_string, parameter_int, parameter_float, out)


class InteractiveTrigger(InteractiveTriggerWithParameters):
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "trigger_action": ("BOOLEAN", {"default": False}),
            },
            "optional": {
                "images": ("IMAGE", ),
            },
    ...
    def run(self, trigger_action, images=None, parameter_latent=None, parameter_mask=None, parameter_string=None, parameter_int=None, parameter_float=None, prompt=None, extra_pnginfo=None):
        ...
        results = save_images_with_metadata(images=images, output_dir=output_dir, save_type="temp", prompt=prompt, extra_pnginfo=extra_pnginfo, prefix=self.prefix_append, compress_level=self.compress_level)
        out["ui"] = {"images": results}
